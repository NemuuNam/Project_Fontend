                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-16 px-2">
                    <div className="flex-1 space-y-4">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white rounded-full shadow-sm border border-slate-100 mb-2 animate-bounce-slow">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#8B7E66]">ประวัติการเคลื่อนไหวสต็อก</span>
                        </div>
                    ...
                    <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-10 mb-14 relative z-10">
                        ...
                        <div className="flex flex-col md:flex-row gap-4 w-full xl:max-w-2xl">
                            <div className="relative flex-1 group">
                                ...
                                <input 
                                    className="w-full pl-14 pr-8 py-4 rounded-full bg-slate-50/50 border border-transparent focus:bg-white focus:border-slate-200 outline-none font-bold text-lg transition-all shadow-inner placeholder:text-[#2D241E]/20" 
                            ...
                            {canClearLogs && (
                                <button onClick={handleClearAll} className="px-8 py-4 bg-white text-red-400 border border-red-50 rounded-full font-black flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all uppercase text-[10px] tracking-widest shadow-sm active:scale-95">
                                    ...
                                    <tr key={log.inv_log_id} className="group/row hover:translate-x-1 transition-all">
                                        ...
                                        <td className="px-10 py-7 rounded-l-[2.5rem] md:rounded-l-[3rem] bg-white border-y border-l border-slate-50 group-hover/row:bg-slate-50/50 whitespace-nowrap">
                                            <div className="flex items-center gap-4">
                                                <div className="p-3 bg-white text-[#2D241E]/20 rounded-2xl shadow-sm border border-slate-100 group-hover/row:text-[#D97706] transition-colors">
                                                    <Calendar size={18} />
                                                </div>
                                        ...
                                        <td className="px-10 py-7 bg-white border-y border-slate-50 group-hover/row:bg-slate-50/50">
                                            <div className="flex items-center gap-4 text-left">
                                                <div className="w-12 h-12 rounded-2xl bg-white border border-slate-100 flex items-center justify-center text-[#2D241E]/20 group-hover/row:text-[#D97706] group-hover/row:border-[#D97706]/20 transition-all shadow-sm shrink-0">
                                                    <Package size={22} />
                                                </div>
                                                <span className="font-black text-lg text-[#2D241E] truncate max-w-[220px] uppercase tracking-tighter italic leading-none">
                                        ...
                                        <td className="px-10 py-7 bg-white border-y border-slate-50 group-hover/row:bg-slate-50/50 text-center">
                                            <div className={`inline-flex items-center gap-2 px-6 py-3 rounded-full text-sm font-black border tracking-tighter shadow-sm transition-transform group-hover/row:scale-110 ${
                                                log.change_qty > 0 
                                        ...
                                        <td className="px-10 py-7 bg-white border-y border-slate-50 group-hover/row:bg-slate-50/50 whitespace-nowrap">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-xl bg-white text-[#2D241E] flex items-center justify-center font-black text-[10px] border-4 border-white shadow-sm overflow-hidden group-hover/row:scale-110 transition-transform">
                                                    {log.user?.first_name?.charAt(0) || <User size={14}/>}
                                                </div>
                                                <div className="flex flex-col text-left">
                                                    <span className="font-black text-[#2D241E] text-sm uppercase tracking-tighter leading-none">
                                                        {log.user ? `${log.user.first_name} ${log.user.last_name || ''}` : 'ระบบอัตโนมัติ'}
                                                    </span>
                                                    <span className="text-[9px] font-black text-[#D97706] uppercase tracking-[0.2em] opacity-70 italic">Authorized Audit</span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-10 py-7 rounded-r-[2.5rem] md:rounded-r-[3rem] bg-white border-y border-r border-slate-50 group-hover/row:bg-slate-50/50 text-left">
                                            <div className="flex items-center justify-between gap-4">
                                                <span className="font-bold text-[#2D241E]/60 text-sm italic group-hover/row:text-[#2D241E] transition-colors line-clamp-1">
                                        ...
                                        <td colSpan="5" className="text-center py-40 bg-white">
                                            <div className="flex flex-col items-center gap-6">
                                                ...
                                                <div className="space-y-1 text-center">
                                                    ...
                                                    <p className="text-sm font-light italic text-[#C2B8A3]">ยังไม่มีการเคลื่อนไหวของสินค้าในระบบ</p>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
