    ...
    return (
        <div className="min-h-screen bg-white font-['Kanit'] text-[#2D241E] relative selection:bg-[#F3E9DC] overflow-x-hidden">
            ...
            <section className="relative pt-20 pb-12 px-6">
                <div className="container mx-auto text-center relative z-10">
                    <div className="inline-flex items-center gap-2 px-4 py-2 bg-white rounded-full shadow-sm border border-slate-100 mb-6">
                        ...
                        <span className="text-[20px] font-black uppercase tracking-[0.1em] text-[#2D241E]">คอลเลกชันพิเศษ</span>
                    </div>
                    <h1 className="text-5xl md:text-7xl font-black text-[#2D241E] mb-10 tracking-tighter uppercase italic leading-none">
                        เมนู <span className="text-[#2D241E] font-light">โฮมเมด</span>
                    </h1>
                    
                    <div className="max-w-2xl mx-auto relative group">
                        <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-[#2D241E] group-focus-within:text-[#2D241E] transition-colors" size={20} />
                        <input 
                            type="text" 
                            placeholder="ค้นหาความอร่อยที่คุณต้องการ..." 
                            className="w-full pl-14 pr-6 py-5 bg-white border border-slate-100 rounded-[2.5rem] text-xl outline-none shadow-sm focus:shadow-md transition-all placeholder:text-[#2D241E]" 
                            value={searchTerm} 
                ...
                <div className="flex flex-col lg:flex-row gap-12">
                    ...
                    <aside className="w-full lg:w-72 shrink-0">
                        <div className="sticky top-32 bg-white p-8 rounded-[3.5rem] border border-slate-100 shadow-sm">
                            ...
                            <div className="space-y-10">
                                <div>
                                    <h3 className="text-[20px] font-black uppercase tracking-[0.1em] text-[#2D241E] mb-6 border-b border-slate-50 pb-3">หมวดหมู่</h3>
                                    <div className="space-y-4 text-left">
                                        {categories.map(cat => (
                                            <label key={cat.category_id} className="flex items-center gap-3 cursor-pointer group">
                                                ...
                                                <span className="text-[20px] font-bold text-[#2D241E] group-hover:text-[#2D241E] transition-colors">{cat.category_name}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div>
                                    <h3 className="text-[20px] font-black uppercase tracking-[0.1em] text-[#2D241E] mb-6 border-b border-slate-50 pb-3">ช่วงราคา</h3>
                                    <div className="space-y-4 text-left">
                                        {[
                                            ...
                                            <label key={range.val} className="flex items-center gap-3 cursor-pointer group">
                                                ...
                                                <span className="text-[20px] font-bold text-[#2D241E] group-hover:text-[#2D241E] transition-colors">{range.label}</span>
                                            </label>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {(selectedCats.length > 0 || priceRange.length > 0) && (
                                <button 
                                    ...
                                    className="mt-10 w-full py-4 bg-slate-50 text-[#2D241E] rounded-2xl font-black text-[20px] uppercase tracking-widest hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-slate-100"
                                >
                                    ล้างค่าตัวกรอง
                                </button>
                            )}
                    ...
                    <div className="flex-1">
                        <div className="mb-8 flex justify-between items-center px-4">
                            <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em]">พบสินค้าทั้งหมด <span className="text-[#2D241E]">{filteredProducts.length} รายการ</span></p>
                        </div>
                        ...
                        {filteredProducts.length > 0 ? (
                                    ...
                                    <div key={product.product_id} className="group bg-white rounded-[3.5rem] p-6 border border-slate-50 hover:shadow-2xl transition-all duration-700 flex flex-col h-full relative overflow-hidden">
                                        ...
                                        <div className="relative aspect-square overflow-hidden rounded-[2.5rem] mb-6 bg-slate-50 cursor-pointer" onClick={() => openProductDetail(product)}>
                                            ...
                                            {product.stock_quantity <= 0 && (
                                                <div className="absolute inset-0 flex items-center justify-center">
                                                    <span className="bg-white/95 text-[#2D241E] px-6 py-2 rounded-full font-black text-[20px] uppercase tracking-widest shadow-xl">สินค้าหมด</span>
                                                </div>
                                            )}
                                        </div>
                                        ...
                                        <div className="px-2 flex flex-col flex-grow text-left">
                                            <div className="flex items-center gap-2 mb-3">
                                                <span className="text-[20px] font-black uppercase tracking-widest text-[#2D241E] bg-amber-50 px-3 py-1 rounded-lg">
                                            ...
                                            <div className="mt-auto flex justify-between items-center pt-6 border-t border-slate-50">
                                                <div className="flex flex-col">
                                                    <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest mb-0.5">ราคา</p>
                                                    <span className="text-2xl font-black text-[#2D241E] tracking-tighter">฿{Number(product.unit_price).toLocaleString()}</span>
                                                </div>
                                                <button 
                                                    onClick={() => addToCart(product)} 
                                                    disabled={product.stock_quantity <= 0} 
                                                    className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all shadow-sm border ${product.stock_quantity > 0 ? 'bg-white border-slate-100 text-[#2D241E] hover:shadow-md active:scale-95' : 'bg-slate-50 text-[#2D241E] cursor-not-allowed'}`}
                                                >
                            ...
                            <div className="text-center py-40 bg-white rounded-[4rem] border border-dashed border-slate-100">
                                ...
                                <h3 className="text-xl font-black text-[#2D241E] uppercase tracking-widest">ไม่พบสินค้าที่ต้องการ</h3>
                            </div>
                        )}
                    </div>
                </div>
                        ...
                        <div className="lg:w-1/2 h-1/2 lg:h-full flex flex-col bg-white">
                            <div className="p-8 lg:p-14 pb-0 shrink-0 text-left">
                                <div className="flex justify-between items-start mb-8">
                                    <div className="space-y-3">
                                        <span className="px-4 py-1.5 bg-slate-50 text-[#2D241E] rounded-xl text-[20px] font-black uppercase tracking-widest">{selectedProduct.category?.category_name}</span>
                                    ...
                                    <button onClick={() => setIsDetailModalOpen(false)} className="w-12 h-12 flex items-center justify-center bg-white text-[#2D241E] hover:text-red-400 rounded-full shadow-sm border border-slate-50 transition-all active:scale-95"><X size={24} /></button>
                                </div>
                                ...
                                <div className="flex gap-10 border-b border-slate-50">
                                    {['detail', 'reviews'].map(tab => (
                                        <button 
                                            ...
                                            className={`pb-5 text-[20px] font-black uppercase tracking-[0.1em] relative transition-all ${activeTab === tab ? 'text-[#2D241E]' : 'text-[#2D241E]'}`}
                                        >
                            ...
                            <div className="flex-1 overflow-y-auto p-8 lg:p-14 custom-scrollbar text-left">
                                {activeTab === 'detail' ? (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                                        <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em] flex items-center gap-2"><Info size={14}/> ข้อมูลสินค้า</p>
                                        <p className="text-base lg:text-xl font-light text-[#2D241E]/80 leading-relaxed whitespace-pre-wrap italic">
                                    ...
                                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
                                        {reviews.length > 0 ? reviews.map((rev, idx) => (
                                            <div key={idx} className="bg-slate-50/50 p-6 rounded-[2.5rem] border border-white shadow-sm">
                                                ...
                                                <p className="text-[#2D241E] text-[20px] italic font-medium">"{rev.comment}"</p>
                                            </div>
                                        )) : (
                                            <div className="text-center py-20 bg-slate-50 rounded-[2.5rem] border border-dashed border-white">
                                                ...
                                                <p className="text-[#2D241E] font-black uppercase tracking-widest text-[20px]">ยังไม่มีการรีวิวสำหรับเมนูนี้</p>
                                            </div>
                                        )}
                                    </div>
                                )}
                            ...
                            <div className="p-8 lg:p-14 border-t border-slate-50 flex items-center justify-between gap-8 shrink-0">
                                <div className="text-left">
                                    <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest mb-1">ราคาต่อหน่วย</p>
                                ...
                                <button 
                                    ...
                                    className="flex-1 bg-white text-[#2D241E] py-6 rounded-full font-black  text-xl uppercase tracking-widest shadow-xl border border-slate-100 hover:shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-4"
                                >
            ...
            <style dangerouslySetInnerHTML={{ __html: `
                ...
                input::placeholder { color: #2D241E; font-weight: 500; font-size: 15px; }
            `}} />
        </div>
    );
};
