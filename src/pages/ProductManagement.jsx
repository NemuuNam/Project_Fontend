                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-16 px-2">
                    <div className="flex-1 space-y-4">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white rounded-full shadow-sm border border-slate-100 mb-2 animate-bounce-slow">
                            ...
                            <span className="text-[20px] font-black uppercase tracking-[0.1em] text-[#2D241E]/60">ศูนย์ควบคุมคลังสินค้า</span>
                        </div>
                    ...
                    <div className="flex flex-wrap gap-4 relative z-20 w-full md:w-auto">
                        <button onClick={() => setModalType('category')} className="flex-1 md:flex-none bg-white text-[#2D241E] border border-slate-200 px-6 py-4 rounded-full font-black text-[20px] uppercase tracking-widest shadow-sm hover:shadow-md hover:-translate-y-1 transition-all flex items-center justify-center gap-3 active:scale-95">
                            <FolderPlus size={16} className="text-[#2D241E]"/> หมวดหมู่สินค้า
                        </button>
                        <button onClick={() => { setIsEditing(false); setModalType('product'); setFormData({ name: '', unitPrice: '', stock: '', category_id: '', image: null, description: '' }); setImagePreview(null); }} className="flex-1 md:flex-none bg-[#2D241E] text-white px-8 py-4 rounded-full font-black text-[20px] uppercase tracking-widest shadow-xl hover:bg-black hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-3">
                    ...
                    {outOfStockCount > 0 && (
                        <div className="bg-white border border-red-50 p-8 rounded-[3rem] shadow-sm flex items-center gap-6 group animate-in fade-in slide-in-from-left-4 duration-700">
                            ...
                            <div>
                                ...
                                <p className="text-[#8B7E66]  text-xl font-light italic">พบ {outOfStockCount} รายการที่ต้องเติมสินค้า</p>
                            </div>
                        </div>
                    )}
                    {lowStockCount > 0 && (
                        <div className="bg-white border border-amber-50 p-8 rounded-[3rem] shadow-sm flex items-center gap-6 group animate-in fade-in slide-in-from-left-4 duration-700">
                            ...
                            <div>
                                ...
                                <p className="text-[#8B7E66]  text-xl font-light italic">พบ {lowStockCount} รายการที่สต็อกต่ำกว่า {LOW_STOCK_THRESHOLD} ชิ้น</p>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-center gap-10 mb-14 relative z-10">
                        <div className="space-y-3 w-full lg:w-auto">
                            <h3 className="text-3xl font-black text-[#2D241E] tracking-tighter uppercase italic flex items-center gap-4">
                                <Package className="text-[#2D241E]" /> Inventory Hub
                            </h3>
                            <div className="flex gap-2 p-1 bg-slate-50/50 rounded-full border border-slate-100 w-fit">
                                {[
                                    ...
                                    <button key={status.id} onClick={() => setFilterStatus(status.id)} className={`px-5 py-1.5 rounded-full text-[20px] font-black uppercase tracking-widest transition-all ${filterStatus === status.id ? 'bg-white text-[#2D241E] shadow-sm' : 'text-[#2D241E] hover:text-[#2D241E]'}`}>{status.label}</button>
                                ))}
                            </div>
                        </div>
                        <div className="relative w-full max-w-xl group">
                            ...
                            <input className="w-full pl-16 pr-8 py-5 rounded-full bg-slate-50/50 border border-transparent focus:bg-white focus:border-slate-200 outline-none font-bold text-xl transition-all shadow-inner placeholder:text-[#2D241E]" placeholder="ค้นหาชื่อสินค้า หรือ หมวดหมู่..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                        </div>
                    </div>
                                        ...
                                        <tr key={p.product_id} className="group/row hover:translate-x-1 transition-all">
                                            <td className="py-6 px-10 rounded-l-[2.5rem] md:rounded-l-[3rem] bg-slate-50/30 group-hover/row:bg-slate-50 border-y border-l border-white">
                                                <div className="flex items-center gap-6">
                                                    <div className="w-16 h-16 rounded-[1.5rem] overflow-hidden border-4 border-white shadow-sm shrink-0 relative bg-white">
                                                        ...
                                                        {isOut && <div className="absolute inset-0 bg-red-500/10 flex items-center justify-center text-[20px] text-red-600 font-black uppercase">Nil</div>}
                                                    </div>
                                                    <div className="flex flex-col min-w-0">
                                                        <span className="font-black text-xl text-[#2D241E] uppercase tracking-tighter truncate max-w-[300px]">{p.product_name}</span>
                                                        <span className="text-[20px] text-[#2D241E] font-light italic truncate max-w-[200px]">{p.description || 'ไม่มีบันทึกข้อมูลรายละเอียด'}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-6 px-10 bg-slate-50/30 group-hover/row:bg-slate-50 border-y border-white">
                                                <span className="px-4 py-1.5 bg-white border border-slate-100 text-[#2D241E] rounded-xl font-black text-[20px] uppercase tracking-widest shadow-sm">{p.category?.category_name}</span>
                                            </td>
                                            <td className="py-6 px-10 bg-slate-50/30 group-hover/row:bg-slate-50 border-y border-white text-right font-black text-[20px] text-[#2D241E] italic leading-none">฿{Number(p.unit_price).toLocaleString()}</td>
                                            <td className="py-6 px-10 bg-slate-50/30 group-hover/row:bg-slate-50 border-y border-white text-center">
                                                <button onClick={() => { setCurrentId(p.product_id); setStockAdjustData({ new_stock: p.stock_quantity, reason: '' }); setModalType('stock'); }} 
                                                    ...
                                                    <span className="font-black text-[20px] leading-none">{p.stock_quantity}</span>
            ...
            {modalType === 'product' && (
                <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 lg:p-10 bg-[#2D241E]/10 backdrop-blur-md animate-in fade-in duration-500">
                    <div className="bg-white w-full max-w-6xl rounded-[3rem] md:rounded-[4rem] shadow-2xl p-8 lg:p-16 overflow-hidden max-h-[95vh] flex flex-col relative animate-in zoom-in-95 border border-white">
                        <button onClick={() => setModalType(null)} className="absolute top-8 right-8 md:top-12 md:right-12 p-3 bg-slate-50 text-[#2D241E] hover:text-red-500 rounded-full transition-all border border-white shadow-sm"><X size={24}/></button>
                        
                        <div className="mb-10 text-left">
                             <p className="text-[#2D241E] font-bold text-[20px] uppercase tracking-[0.1em] mb-2 italic">สร้างข้อมูลสินค้า</p>
                             <h2 className="text-3xl lg:text-5xl font-black uppercase tracking-tighter text-[#2D241E] italic">จัดการ <span className="font-light">ข้อมูลสินค้า</span></h2>
                        </div>
                        ...
                        <form onSubmit={handleProductSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-12 flex-1 overflow-y-auto pr-2 custom-scrollbar text-left">
                            <div className="space-y-8">
                                <div className="space-y-2">
                                    <label className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest ml-4">ชื่อเรียกสินค้า</label>
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest ml-4">ราคาสินค้า (฿)</label>
                                        <input type="number" className="w-full p-6 rounded-full bg-slate-50/50 border border-slate-100 outline-none font-black text-2xl shadow-inner text-[#2D241E] focus:bg-white transition-all italic" value={formData.unitPrice} onChange={e => setFormData({...formData, unitPrice: e.target.value})} required />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest ml-4">สต็อกเริ่มต้น</label>
                                ...
                                <div className="space-y-2">
                                    <label className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest ml-4">หมวดหมู่สินค้า</label>
                                    <select className="w-full p-6 rounded-full bg-slate-50/50 border border-slate-100 outline-none font-bold text-xl appearance-none focus:bg-white transition-all shadow-inner text-[#2D241E]" value={formData.category_id} onChange={e => setFormData({...formData, category_id: e.target.value})} required>
                                        <option value="">เลือกหมวดหมู่สินค้า</option>
                                        {categories.map(c => <option key={c.category_id} value={c.category_id}>{c.category_name}</option>)}
                                    </select>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest ml-4">รายละเอียดสินค้า</label>
                                    <textarea className="w-full p-8 rounded-[2.5rem] bg-slate-50/50 border border-slate-100 outline-none font-medium text-xl h-40 resize-none transition-all shadow-inner focus:bg-white text-[#2D241E] italic" placeholder="ข้อมูลวัตถุดิบ หรือ ข้อมูลอื่นๆ..." value={formData.description} maxLength={255} onChange={e => setFormData({...formData, description: e.target.value})} />
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-10">
                                <div className="flex flex-col items-center justify-center bg-slate-50/30 rounded-[3rem] border-4 border-dashed border-slate-50 p-8 flex-1 min-h-[350px] relative overflow-hidden group/upload">
                                    ...
                                    <label className="px-10 py-4 bg-white text-[#2D241E] border border-slate-200 rounded-full font-black uppercase text-[20px] tracking-[0.1em] cursor-pointer hover:shadow-lg transition-all active:scale-95 relative z-10">
                                        เลือกรูปภาพ
                                        <input type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
                                    </label>
                                </div>
                                <button type="submit" disabled={isUploading} className="py-6 bg-[#2D241E] text-white rounded-full font-black text-xl shadow-xl hover:bg-black transition-all flex justify-center items-center gap-4 active:scale-95 uppercase tracking-[0.1em]">
            ...
            {modalType === 'stock' && (
                <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-[#2D241E]/10 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-[3rem] md:rounded-[4rem] shadow-2xl p-10 md:p-12 relative animate-in zoom-in-95 border border-white text-center">
                        <button onClick={() => setModalType(null)} className="absolute top-10 right-10 p-3 bg-slate-50 text-[#2D241E] hover:text-red-500 rounded-full transition-all border border-white shadow-sm"><X size={20}/></button>
                        <div className="w-20 h-20 bg-slate-50 text-[#2D241E] rounded-[2.2rem] flex items-center justify-center mx-auto mb-8 border border-white shadow-inner"><ArrowRightLeft size={36} /></div>
                        ...
                        <form onSubmit={handleStockAdjustment} className="space-y-8 mt-10">
                            <div className="relative">
                                ...
                                <span className="absolute bottom-[-25px] left-1/2 -translate-x-1/2 text-[9px] font-black uppercase tracking-widest text-[#2D241E]">จำนวนหน่วยใหม่</span>
                            </div>
                            <textarea className="w-full p-6 rounded-[2.5rem] bg-slate-50/50 border border-slate-100 font-medium text-xl h-32 resize-none shadow-inner focus:bg-white outline-none text-[#2D241E] italic" placeholder="เหตุผลในการปรับปรุง..." value={stockAdjustData.reason} onChange={e => setStockAdjustData({...stockAdjustData, reason: e.target.value})} required />
                            <button type="submit" className="w-full py-6 bg-[#2D241E] text-white rounded-full font-black text-[20px] hover:opacity-90 transition-all uppercase tracking-[0.1em] shadow-xl active:scale-95">ยืนยันการปรับปรุง</button>
                        </form>
                    </div>
                </div>
            )}
            ...
            {modalType === 'category' && (
                <div className="fixed inset-0 z-[500] flex items-center justify-center p-6 bg-[#2D241E]/10 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-[3rem] md:rounded-[4rem] shadow-2xl p-10 md:p-12 relative max-h-[85vh] flex flex-col animate-in zoom-in-95 border border-white">
                        ...
                        <div className="mb-10 text-left">
                            <p className="text-[#2D241E] font-bold text-[20px] uppercase tracking-[0.1em] mb-2 italic">สร้างหมวดหมู่สินค้า</p>
                        ...
                        <form onSubmit={handleCategorySubmit} className="space-y-6 mb-8 pb-8 border-b border-slate-50">
                            <div className="flex gap-4">
                                <input className="flex-1 p-5 rounded-full bg-slate-50/50 border border-slate-100 font-bold text-xl focus:bg-white outline-none text-[#2D241E] shadow-inner placeholder:text-[#2D241E]/20" value={categoryName} onChange={e => setCategoryName(e.target.value)} placeholder="ชื่อหมวดหมู่ใหม่..." required />
                                ...
                                {categories.map(c => (
                                    <div key={c.category_id} className="flex justify-between items-center bg-white p-6 rounded-[2rem] group border border-slate-100 shadow-sm hover:shadow-md transition-all">
                                        <div className="flex items-center gap-4 text-left">
                                            <div className="w-1 h-6 bg-[#2D241E] rounded-full opacity-30"></div>
                                            <span className="font-black text-[#2D241E] text-xl uppercase tracking-tighter italic leading-none">{c.category_name}</span>
                                        </div>
                                        <button onClick={() => handleDeleteCategory(c.category_id)} className="text-[#2D241E] hover:text-red-500 transition-all p-2 rounded-xl active:scale-90">
                                            <Trash2 size={20} />
                                        </button>
                                    </div>
                                ))}
                        ...
                        <button onClick={() => setModalType(null)} className="mt-8 w-full py-4 bg-slate-50 text-[#8B7E66] rounded-full font-black uppercase text-[20px] tracking-[0.1em] hover:text-[#2D241E] transition-all">ปิดหน้าต่าง</button>
                    </div>
                </div>
            )}
