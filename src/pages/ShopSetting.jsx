                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10 mb-16 px-2">
                    <div className="flex-1 space-y-4">
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-white rounded-full shadow-sm border border-slate-100 animate-bounce-slow">
                            ...
                            <span className="text-[20px] font-black uppercase tracking-[0.1em] text-[#2D241E]/60">ตั้งค่าร้านค้าทั่วไป</span>
                        </div>
                        ...
                        <div className="bg-white p-8 md:p-14 rounded-[3rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm relative overflow-hidden group transition-all duration-500 hover:shadow-md">
                            ...
                            <div className="flex justify-between items-start mb-16 relative z-10">
                                <div className="space-y-1">
                                    <h2 className="text-3xl md:text-4xl font-black uppercase tracking-tighter italic text-[#2D241E]">ข้อมูล <span className="opacity-90 font-light">พื้นฐานร้านค้า</span></h2>
                                    <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em]">ข้อมูลหลักที่จะแสดงผลบนหน้าเว็บไซต์</p>
                                </div>
                                <button onClick={() => setActiveModal('general')} className="w-12 h-12 md:w-14 md:h-14 bg-white text-[#2D241E] border border-slate-200 rounded-2xl flex items-center justify-center hover:bg-slate-50 transition-all active:scale-90 shadow-sm group/btn">
                                    <Edit3 size={20} className="group-hover/btn:text-[#2D241E] transition-colors" />
                                </button>
                            </div>
                        ...
                        <div className="bg-white p-8 md:p-14 rounded-[3rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm transition-all duration-500 hover:shadow-md">
                            <div className="flex justify-between items-center mb-16">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em]">จัดการเลขบัญชีธนาคารสำหรับโอนเงิน</p>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                {paymentMethods.map(m => (
                                    <div key={m.method_id} className="p-8 md:p-10 bg-white rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-100 group relative hover:shadow-lg transition-all duration-700">
                                        <button onClick={() => handleDelete('payment', m.method_id)} className="absolute top-8 right-8 p-2 text-[#2D241E] hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={20} /></button>
                                        ...
                                        <p className="text- [#2D241E] font-bold text-[20px] uppercase tracking-[0.1em] mb-10">{m.account_name}</p>
                                ...
                                {paymentMethods.length === 0 && (
                                    <div className="md:col-span-2 py-24 text-center border-2 border-dashed border-slate-100 rounded-[3rem] flex flex-col items-center gap-6">
                                        ...
                                        <p className="text-[#2D241E] font-black uppercase tracking-[0.1em] italic  text-xl">ยังไม่มีข้อมูลช่องทางรับเงิน</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="space-y-6 mb-12">
                                    <div className="bg-slate-50/50 p-10 rounded-[3rem] border border-white hover:bg-white transition-all duration-500 shadow-inner hover:shadow-md text-center">
                                        <p className=" text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em] mb-4 flex items-center justify-center gap-2 leading-none"><Truck size={24} /> ค่าส่งปกติ</p>
                                        <p className="text-3xl md:text-4xl font-black italic tracking-tighter text-[#2D241E] leading-none">฿{formData.delivery_fee}</p>
                                    </div>
                                    <div className="bg-slate-50/50 p-10 rounded-[3rem] border border-white hover:bg-white transition-all duration-500 shadow-inner hover:shadow-md text-center">
                                        <p className=" text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em] mb-4 flex items-center justify-center gap-2 leading-none"><ShieldCheck size={24} /> ส่งฟรีเมื่อซื้อ</p>
                                        <p className="text-3xl md:text-4xl font-black italic tracking-tighter text-[#2D241E] leading-none">{formData.min_free_shipping} <span className="text-xl not-italic opacity-90">ชิ้น</span></p>
                                    </div>
                                </div>

                                <button onClick={() => setActiveModal('shipping_cost')} className="w-full py-6 bg-slate-100 text-black rounded-full font-black text-[20px] uppercase tracking-[0.1em] transition-all hover:bg-white shadow-xl active:scale-95">ตั้งค่า ค่าส่ง</button>
                            </div>
                        </div>
                        ...
                        <div className="bg-white p-8 md:p-12 rounded-[3rem] md:rounded-[4.5rem] border border-slate-100 shadow-sm relative transition-all duration-500 hover:shadow-md">
                            <div className="flex justify-between items-center mb-12 px-2">
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[20px] font-black text-[#2D241E] uppercase tracking-widest">บริษัทขนส่งที่เปิดใช้งาน</p>
                                </div>
                                ...
                                {providers.map(p => (
                                    <div key={p.provider_id} className="flex justify-between items-center p-6 bg-white border border-slate-100 rounded-[2.5rem] hover:shadow-md transition-all group/prov">
                                        <div className="flex items-center gap-4 text-left">
                                            ...
                                            <span className="font-black text-[#2D241E] uppercase  text-xl tracking-widest italic leading-none">{p.provider_name}</span>
                                        </div>
                                        <button onClick={() => handleDelete('provider', p.provider_id)} className="text-[#2D241E] hover:text-rose-500 transition-colors p-2 active:scale-90"><Trash2 size={18} /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="bg-white w-full max-w-xl rounded-[3rem] md:rounded-[4rem] shadow-2xl overflow-hidden animate-in zoom-in-95 border border-white" onClick={e => e.stopPropagation()}>
                        <div className="p-10 md:p-14 relative">
                            ...
                            <div className="mb-12 text-left">
                                <p className="text-[#2D241E] font-bold text-[20px] uppercase tracking-[0.6em] mb-2 italic leading-none">Configuration Master</p>
                                <h2 className="text-3xl md:text-4xl font-black text-[#2D241E] uppercase tracking-tighter italic leading-none">ปรับแต่ง <span className=" font-light">ข้อมูลระบบ</span></h2>
                            </div>
                            ...
                            <div className="max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar text-left">
                                {activeModal === 'general' && (
                                    <form onSubmit={handleUpdate} className="space-y-8">
                                        ...
                                        <div className="space-y-2">
                                            <label className=" text-[20px] font-black uppercase text- [#2D241E] ml-6 tracking-[0.1em]">ที่อยู่สำนักงาน / ร้านค้า</label>
                                            <textarea className="w-full p-8 rounded-[2.5rem] bg-slate-50/50 border border-slate-100 outline-none font-bold text-xl h-32 resize-none focus:bg-white focus:border-[#2D241E]/10 transition-all shadow-inner text-[#2D241E] italic" value={formData.address} onChange={e => setFormData({ ...formData, address: e.target.value })} placeholder="ระบุบ้านเลขที่ ถนน แขวง/ตำบล..." />
                                        </div>
                                        <button type="submit" className="w-full py-6 bg-[#ffff] text-#2D241E rounded-full font-black text-xl uppercase tracking-[0.1em] shadow-sm hover:bg-slate-50 transition-all active:scale-95">บันทึกข้อมูลทั่วไป</button>
                                    </form>
                                )}
                                ...
                                {activeModal === 'payments' && (
                                    <div className="space-y-8">
                                        ...
                                        <button onClick={handleAddPayment} className="w-full py-6 bg-[#ffff] text-#2D241E rounded-full font-black text-xl uppercase tracking-[0.1em] shadow-sm hover:bg-slate-50 transition-all active:scale-95">เพิ่มบัญชีธนาคาร</button>
                                    </div>
                                )}
                                ...
                                {activeModal === 'shipping_cost' && (
                                    <form onSubmit={handleUpdate} className="space-y-10">
                                        ...
                                        <button type="submit" className="w-full py-6 bg-[#ffff] text-#2D241E rounded-full font-black text-xl uppercase tracking-[0.1em] shadow-sm hover:bg-slate-50 transition-all active:scale-95">ยืนยันเงื่อนไขค่าส่ง</button>
                                    </form>
                                )}
                                ...
                                {activeModal === 'providers' && (
                                    <div className="space-y-8">
                                        ...
                                        <button onClick={handleAddProvider} className="w-full py-6 bg-[#ffff] text-#2D241E rounded-full font-black text-xl uppercase tracking-[0.1em] shadow-sm hover:bg-slate-50 transition-all active:scale-95">เพิ่มรายชื่อขนส่ง</button>
                                    </div>
                                )}
                            </div>
                        </div>
...
const StatCardWhite = ({ title, value, icon, color }) => (
    <div className="bg-white p-8 rounded-[2.5rem] md:rounded-[3rem] border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition-all hover:-translate-y-1.5 duration-500 group relative overflow-hidden">
        <div className="flex-1 text-left min-w-0 pr-4 relative z-10">
            <p className=" text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em] mb-4 flex items-center gap-2 leading-none">
...
const InfoBlock = ({ label, value, icon, isFull = false }) => (
    <div className={`space-y-3 ${isFull ? 'md:col-span-2' : ''} group/item`}>
        <label className="text-[20px] font-black text-[#2D241E] uppercase tracking-[0.1em] flex items-center gap-2 ml-2 transition-colors group-hover/item:text-[#2D241E]">{icon} {label}</label>
        <p className={`text-[#2D241E] font-black border-b border-slate-50 pb-5 leading-tight transition-all group-hover/item:border-[#2D241E] ${isFull ? 'text-xl md:text-2xl font-bold' : 'text-2xl md:text-3xl tracking-tighter'}`}>{value || '—'}</p>
    </div>
);

const ModalInputField = ({ label, value, onChange, type = "text", placeholder = "", required = false, isHuge = false }) => (
    <div className="space-y-2 group/field">
        <label className=" text-[20px] font-black uppercase text- [#2D241E] ml-6 tracking-[0.1em] transition-colors group-focus-within/field:text-[#2D241E]">{label}</label>
        <input
            type={type}
            className={`w-full p-6 rounded-full bg-slate-50/50 border border-slate-100 outline-none font-bold transition-all shadow-inner focus:bg-white focus:border-[#2D241E] text-[#2D241E] italic ${isHuge ? 'text-6xl md:text-7xl text-center tracking-tighter' : 'text-xl'}`}
            value={value}
